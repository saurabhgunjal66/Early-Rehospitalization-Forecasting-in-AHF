...
class SyntheticDataGenerator:
    ...
    def generate_patient_variants(self, base_patient, n_variants=10):
        """Generate variants of a patient with slight parameter changes."""
        variants = []
        ...
        for i in range(n_variants):
            ...
            if 'nt_probnp' in variant:
                variant['nt_probnp'] *= np.random.normal(1.0, 0.15)
                variant['nt_probnp'] = max(50, min(50000, variant['nt_probnp']))
            
            if 'weight' in variant:
                variant['weight'] += np.random.normal(0, 2)
                variant['weight'] = max(40, min(150, variant['weight']))
            
            if 'b_line_score' in variant:
                variant['b_line_score'] += np.random.randint(-2, 3)
                variant['b_line_score'] = max(0, min(28, variant['b_line_score']))
            
            if 'ejection_fraction' in variant:
                variant['ejection_fraction'] += np.random.randint(-5, 6)
                variant['ejection_fraction'] = max(10, min(75, variant['ejection_fraction']))
            
            # Update patient ID
            variant['patient_id'] = f"{base_patient.get('patient_id', 'VAR')}_{i+1}"
    ...
    def get_dataset_summary(self, df):
        ...
        summary = {
            ...
            'readmission_rate': df['readmission_30d'].mean() if 'readmission_30d' in df.columns else 'N/A',
            'demographics': {
    ...
    def generate_time_series_data(self, patient_id, n_timepoints=10, days_interval=1):
        """Generate time series data for a patient over multiple assessments."""
        timepoints = []
        base_date = pd.Timestamp.now() - pd.Timedelta(days=n_timepoints * days_interval)
        
        # Generate base patient
        base_patient_data = self.generate_training_dataset(1).iloc[0].to_dict()
        base_patient_data['patient_id'] = patient_id
        
        for i in range(n_timepoints):
            timepoint_data = base_patient_data.copy()
            
            # Add temporal trends (e.g., disease progression)
            progression_factor = i / n_timepoints
            
            # NT-proBNP might increase over time in worsening HF
            if np.random.random() < 0.3:  # 30% chance of progression
                timepoint_data['nt_probnp'] *= (1 + progression_factor * 0.5)
            
            # Weight might fluctuate due to fluid retention
            weight_change = np.random.normal(0, 2)
            timepoint_data['weight'] += weight_change
            
            # B-lines might vary with fluid status
            if weight_change > 2:  # Weight gain suggests fluid retention
                timepoint_data['b_line_score'] += np.random.randint(1, 4)
            elif weight_change < -2:  # Weight loss suggests diuresis
                timepoint_data['b_line_score'] = max(0, timepoint_data['b_line_score'] - np.random.randint(1, 3))
            
            # Add timestamp
            timepoint_data['assessment_date'] = base_date + pd.Timedelta(days=i * days_interval)
            
            # Ensure values are within valid ranges
            timepoint_data['nt_probnp'] = max(50, min(50000, timepoint_data['nt_probnp']))
            timepoint_data['weight'] = max(40, min(150, timepoint_data['weight']))
            timepoint_data['b_line_score'] = max(0, min(28, timepoint_data['b_line_score']))
            
            timepoints.append(timepoint_data)
    ...
    def create_high_risk_cohort(self, n_samples=100):
        """Generate a cohort of high-risk patients for testing alert system."""
        np.random.seed(self.random_seed + 999)
        
        # Generate base cohort
        cohort = self.generate_training_dataset(n_samples)
        
        # Modify parameters to increase risk
        cohort['nt_probnp'] *= np.random.uniform(2.0, 4.0, n_samples)
        cohort['ejection_fraction'] *= np.random.uniform(0.6, 0.9, n_samples)
        cohort['b_line_score'] += np.random.randint(5, 10, n_samples)
        cohort['age'] += np.random.randint(5, 15, n_samples)
        
        # Increase comorbidity prevalence
        cohort['diabetes'] = np.random.choice([0, 1], n_samples, p=[0.2, 0.8])
        cohort['ckd'] = np.random.choice([0, 1], n_samples, p=[0.3, 0.7])
        cohort['afib'] = np.random.choice([0, 1], n_samples, p=[0.4, 0.6])
        
        # Ensure values stay within valid ranges
        cohort['nt_probnp'] = np.clip(cohort['nt_probnp'], 50, 50000)
        cohort['ejection_fraction'] = np.clip(cohort['ejection_fraction'], 10, 75)
        cohort['b_line_score'] = np.clip(cohort['b_line_score'], 0, 28)
        cohort['age'] = np.clip(cohort['age'], 18, 95)
        
        # Recalculate readmission probability
        readmission_prob = self._calculate_readmission_probability(cohort)
        cohort['readmission_30d'] = np.random.binomial(1, readmission_prob)
        
        print(f"Generated high-risk cohort: {cohort['readmission_30d'].mean():.1%} readmission rate")
